...
生命周期为 `building`，现目录 `goldenone002-building/`（旧名 `goldenone002/`）。
已授权改动完成验证后合入并 push 本仓库 main，允许既有 Actions 发布，不再问 push；
临时分支不是最终交付。该次明确“不推/只做分支”优先。测试站可访问、绑定域名或
内容标为 production-ready，不等于商业上线；独立克隆也按此身份直到站长明确变更。
在现有进度记修改前 SHA、本轮提交、检查、push/CI 与反向提交回退步骤，不授权 DNS、
真实数据库迁移或其它客户操作。纯治理/路径变化只做相关检查，不重跑完整建站。
客户/站长确认的具体事实直接采用，不重复索要外部证明；AI 负责研究/视觉/SEO 字段，
只核对型号、单位、缺项和矛盾，不新增逐字段举证，不编造缺失数字。
factsVerified 可依据相应范围的确认，搜索展示资格另作技术判断。
...
已启用、预览验证、生产发布分别记。获准后做最小依赖适配，必要时用临时分支，
测试旧数据与回滚，最终按上述 building 授权合入并 push main；细则读母版主指令第 13 节。
