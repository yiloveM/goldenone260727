...
本站仍在建设中，本地目录为 `goldenone002-building/`（原 `goldenone002/`），
独立远端仍是 `yiloveM/goldenone260727`。已授权能力修改验证后直接交付到 main，
无需再确认 push；临时分支不能代替 main 交付。商业上线由站长另行明确，
不是因为测试地址能打开就自动变为 online。具体规则见 [AGENTS.md](AGENTS.md)。
...
1. **品牌与行业基础**：维护客户或站长已确认的公司资料、市场和产品；关键词研究、竞品、视觉方向及研究日期由 AI 完成，不要求客户逐项填。客户明确确认即可作为事实依据，无需反复提供外部证明；既有 factsVerified 按确认范围使用。本次未增加资料字段，可选价格/评价缺失不阻止建设询价站。
