...
const contracts = [
  ...
  {
    area: 'Old-site and preview runtime dependencies',
    file: 'package.json',
    markers: ['oldsite:crawl', 'oldsite:prepare', 'oldsite:audit', 'preview:deploy', 'check:preview', 'check:visual', 'check:class-names', 'undici'],
  },
  {
    area: 'Isolated preview branch deployment gate',
    file: 'scripts/run-preview-deploy.mjs',
    markers: ['GITHUB_REF_NAME', "branch === 'main'", "mode === 'current'", "'--preview-alias'", "'DEPLOYMENT_CONTEXT:preview'", "workerName.endsWith('-preview')"],
  },
  {
    area: 'Isolated Cloudflare branch preview workflow',
    file: '.github/workflows/site-preview.yml',
    markers: ["'preview/**'", "github.ref_name != 'main'", 'npm run check:preview', 'npm run build', 'npm run preview:deploy', 'CLOUDFLARE_API_TOKEN'],
  },
  {
    area: 'Preview runtime write and indexing isolation',
    file: 'src/worker.ts',
    markers: ['DEPLOYMENT_CONTEXT', 'previewMutationBlocked', 'securePreviewResponse', "'x-robots-tag': 'noindex, nofollow, noarchive, nosnippet'", 'if (!previewRuntime)'],
  },
  {
...
const requiredFiles = [
  ...
  'scripts/audit-preview-deployment.mjs',
  'scripts/audit-public-visuals.mjs',
  'scripts/audit-generic-class-names.mjs',
