...
const structuralChecks = [
  ...
  ['package.json', ['oldsite:crawl', 'oldsite:prepare', 'oldsite:audit', 'preview:deploy', 'check:preview', 'check:visual', 'check:class-names', 'undici']],
  ...
  ['.github/workflows/site-preview.yml', ["'preview/**'", "github.ref_name != 'main'", 'npm run check:preview', 'npm run preview:deploy', 'CLOUDFLARE_API_TOKEN']],
  ['scripts/run-preview-deploy.mjs', ['GITHUB_REF_NAME', "branch === 'main'", "mode === 'current'", "'--preview-alias'", "'DEPLOYMENT_CONTEXT:preview'", "workerName.endsWith('-preview')"]],
  ['scripts/audit-preview-deployment.mjs', ['site-preview.yml', 'preview_urls', 'securePreviewResponse', 'previewMutationBlocked']],
