...
import { encodeGitHubPath, getBranch, getDispatchToken, getRepoFullName, githubHeaders } from '../../../../lib/manager/github';
...
const readRepoFile = async (repoFullName: string, branch: string, path: string, accessToken: string) => {
  const response = await fetch(
    ...
    { headers: githubHeaders(accessToken, 'businessweb-manager-draft-preview') }
  );
...
const safeDraftSlug = (value: string | null) => {
  ...
  return /^[a-z0-9]+(?:-[a-z0-9]+)*(?:--[a-z0-9]+(?:-[a-z0-9]+)*)?$/.test(draft) ? draft : '';
};

const slugFromId = (id: string) => id.replace(/\.mdoc$/, '');

const readLocalDraft = async (type: DraftType, draftSlug: string) => {
  const entries =
    type === 'product'
      ? await getCollection('productTranslations')
      : await getCollection('blogTranslations');
  const entry = entries.find(item => slugFromId(item.id) === draftSlug);
...
const readLocalSource = async (type: DraftType, sourceSlug: string) => {
  if (!sourceSlug) return {};
  const entries = type === 'product' ? await getCollection('products') : await getCollection('blog');
  const entry = entries.find(item => slugFromId(item.id) === sourceSlug);
...
const renderSpecTables = (tables: any[] = []) =>
  tables
    ...
    .map(table => {
      const headerRows =
        Array.isArray(table.headerRows) && table.headerRows.length
          ? table.headerRows
...
<html lang="${escapeHtml(locale)}">
  ...
  <body>
    <main class="preview-shell">
      <div class="preview-banner">草稿预览页面。确认内容无误后回到管理后台勾选草稿并一键审核。</div>
...
export const GET: APIRoute = async ({ locals, request, url }) => {
  ...
  if (accessToken) {
    ...
    try {
      ...
      if (draftFile) {
        draft = splitMdoc(draftFile);
        const sourceSlug = String(draft.data.sourceSlug || '');
        const sourcePath = type === 'product' ? `src/content/products/${sourceSlug}.mdoc` : `src/content/blog/${sourceSlug}.mdoc`;
        const sourceFile = sourceSlug ? await readRepoFile(repoFullName, branch, sourcePath, accessToken) : null;
        source = sourceFile ? splitMdoc(sourceFile).data : {};
      }
    } catch (error) {
      void error;
    }
  ...
  if (!draft) {
    draft = await readLocalDraft(type, draftSlug);
    if (!draft) return new Response('找不到这个翻译草稿。', { status: 404 });
    source = await readLocalSource(type, String(draft.data.sourceSlug || ''));
  }
  ...
  const body =
    type === 'product'
      ? [
          ...
          .filter(Boolean)
          .join('')
      : [`<section class="preview-card"><span class="eyebrow">${escapeHtml(data.category)}</span>${renderMarkdownPreview(draft.body)}</section>`].join('');
