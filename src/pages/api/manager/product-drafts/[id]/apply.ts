...
const WORKFLOW_FILE = 'manager-apply-product-draft.yml';
...
export const POST: APIRoute = async ({ locals, request, params }) => {
  ...
  if (!/^product-[a-z0-9-]+-[a-z0-9]+$/.test(draftId)) {
  ...
  if (!record) {
    return new Response('Product draft was not found.', {
      status: 404,
      headers: { 'cache-control': 'no-store', 'content-type': 'text/plain; charset=utf-8' },
    });
  }
  ...
  const requestId = createRequestId('manager');
  ...
  return new Response(
    JSON.stringify({
      ok: true,
      requestId,
      draft: productDraftToResponse({ ...record, status: 'queued', workflow_request_id: requestId, workflow_url: actionsUrl }),
    }),
    {
      headers: {
